...
paramC = 750/75;
setPointThreshold = header.IRelaxR;
errorThreshold = header.IRelaxE;
...
subplot(2,1,2);
hold on
plot(meas.sysTime, [meas.PIDPoutX/(header.Px/1000) ]);
...
clf(10);
subplot(2,1,1);
uX = meas.PIDPoutX+meas.PIDIoutX+meas.PIDDoutX;
% xline(t_start);
% xline(t_end);
hold on
plot(meas.sysTime, [meas.PIDRefX meas.PIDSensX uX*10]);
ylim([-400 400]);
...
figure(11);
clf(11);
plot(meas.sysTime, [meas.PIDPoutX meas.PIDIoutX meas.PIDDoutX]);
yline(0,'--');
